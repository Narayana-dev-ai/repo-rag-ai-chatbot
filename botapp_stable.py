...
def initialize_vectorstore():
    ...
    try:
        ...
        print("Loaded vectorstore from disk....Go on")
    except Exception as e:
        # If loading fails, log the error and create a new vectorstore
        print(f"Error loading vectorstore so we are creating Fresh one:{e}")
        print("So Now Starting fresh vectorstore database creation...")
        print("Reading the PDF's...")
        raw_text = read_pdfs()  # Read all PDFs
        print("Creating Chunks from texts...")
        text_chunks = get_text_chunks(raw_text)  # Get text chunks
        print("Creating vector...")
        vectorstore = load_or_create_vectorstore(text_chunks, embeddings)
...
def get_text_chunks(text):
    ...
    text_splitter = CharacterTextSplitter(
        ...
        chunk_size=1500,
        chunk_overlap=100,
        length_function=lambda x: min(len(x), 1000) #hard limit on chunk size
    )
    chunks = text_splitter.split_text(text)
    return chunks

def load_or_create_vectorstore(text_chunks, embeddings):
    """Load or create vectorstore."""
    vectorstore_folder = Path("faiss_doc")
    try:
        vectorstore = FAISS.load_local(vectorstore_folder, embeddings=embeddings, allow_dangerous_deserialization=True)
    except:
        # Handle error during loading and create a new vector store
        print("Vector creation process will take several minutes...")
        vectorstore = create_vectorstore(text_chunks, embeddings)
        print("Vector created successfully, Now saving it....")
        vectorstore.save_local(vectorstore_folder)
        print("Vector saved in faiss_doc folder !")
    return vectorstore
...
def get_conversation(vectorstore):
    ...
    llm = OllamaLLM(model="llama3.2:1b", temperature=0.2, max_tokens=100, callbacks=callback_manager)
...
def handle_userinput(user_question):
    ...
    for message in chat_history:
        if isinstance(message, AIMessage):
            output.append({"role": "AI", "content": message.content})
        else:
            output.append({"role": "Human", "content": message.content})
...
def get_hashable_key(text_chunks):
    """Generate a hashable key from text chunks."""
    # Convert the list of text chunks to a string and hash it
    text_str = json.dumps(text_chunks, ensure_ascii=False)
