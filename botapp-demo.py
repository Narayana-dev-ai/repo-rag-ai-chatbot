...
def load_pretrained_answers():
    """Load predefined question-answer pairs from pre_trained.txt."""
    preloaded_answers = {}
    file_path = Path("pre_trained.txt")
    
    # Check if the file exists
    if not file_path.exists():
        # Create the file with default content
        with open(file_path, 'w', encoding='utf-8') as file:
            file.write("What is CMI?;CMI stands for Configuration Management Interface, used for managing configurations.\n")
            print("[INFO] pre_trained.txt file created with default content.")
    
    # Load the answers from the file
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            question, answer = line.strip().split(";", 1)
            preloaded_answers[question] = answer
            
    return preloaded_answers

# Load preloaded answers at startup
preloaded_answers = load_pretrained_answers()

def normalize_input(input_string):
    """Normalize the input by removing special characters and sorting words."""
    # Remove special characters using regex
    cleaned_input = re.sub(r'[^a-zA-Z0-9\s]', '', input_string)
    # Normalize by converting to lowercase, splitting, and sorting
    normalized = ' '.join(sorted(cleaned_input.lower().split()))
    return normalized
...
def chat():
    ...
    normalized_user_question = normalize_input(user_question)

    # Check for exact matches with the preloaded answers
    for preloaded_question in preloaded_answers.keys():
        normalized_preloaded_question = normalize_input(preloaded_question)
        if normalized_user_question == normalized_preloaded_question:
            response_content = preloaded_answers[preloaded_question]
            save_interaction(user_question, response_content)
            return jsonify({
                "content": preloaded_answers[preloaded_question],
                "role": "Human"
            })
...
def get_text_chunks(text):
    ...
    text_splitter = CharacterTextSplitter(
        separator="\n",
        chunk_size=50,  # Lower chunk size for better retrieval
...
def get_conversation(vectorstore):
    ...
    llm = OllamaLLM(model="llama3.2:1b", temperature=0.2, max_tokens=50, callbacks=callback_manager)

    # Updated the prompt for concise answers
    prompt = PromptTemplate.from_template(
        """
        You are a helpful AI assistant trained on user manuals and FAQs for the CMI project.
        Provide direct and concise answers (2-3 lines) based on the context below. 
...
def save_interaction(user_question, ai_response, user_answer=None):
    ...
    with open('chat_history/user_interactions.txt', 'a', encoding='utf-8') as f:
        ...
        # Optionally add the user question and response directly to the vector store
        if ADD_INTERACTIONS_TO_VECTORS:
            add_if_new_to_vector_store(ai_response)  # Store the AI response directly
